...
def live_env(sess):
    ...
    # archived sessions can carry stale connected flags on dead envs
...
def cmd_run():
    todo = [o for o in wedged_map(sessions()) if o['transcript']]
    state = json.load(open(STATE)) if os.path.exists(STATE) else {}
    env = live_env(sessions())
...
if __name__ == '__main__':
    ...
    a = p.parse_args()
    {'list': cmd_list, 'plan': cmd_plan, 'run': cmd_run,
